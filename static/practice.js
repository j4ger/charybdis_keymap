...
let practice = { active:false, text:'', typed:[], pos:0, startMs:0, correct:0, errors:0, finished:false, emul:true, numbers:false, symbols:false };
let posToChar = {}, charToPos = {};
...
function generatePrompt(n, numbers, symbols){
  var SYMS = ['.', ',', ';', ':', '!', '?'];
  var out = [];
  for(var i=0;i<n;i++){
    if(numbers && Math.random() < 0.15){
      ...
      if(symbols && Math.random() < 0.30){
        w += SYMS[Math.floor(Math.random()*SYMS.length)];
      }
      out.push(w);
    }
  }
...
function pickWords(n){ return generatePrompt(n, false, false); }
...
function startPractice(){
  ...
  practice.text = generatePrompt(n, practice.numbers, practice.symbols);
...
function handleBackspace(){
  if(practice.pos > 0){
    practice.pos--; practice.typed.pop();
...
function onKeydown(e){
  ...
  if(practice.emul && onBase){
    var pk = CODE_TO_POS[e.code]; ch = pk ? posToChar[pk] : null;
  } else {
